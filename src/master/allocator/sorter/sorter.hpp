...
public:
  ...
  // yuquan: add total resources to the sorter, I would let it be non-virtual
  // TODO(yuquan): also need to add removeSlaveTotal() when the slave is removed
  // from the cluster
  ...
  virtual std::vector<std::string> sortPerSlave(const SlaveID& slaveId) = 0;
  // yuquan: overload function sort here
