...
struct Client
{
  ...
  // Client(const std::string& _name, double _share, uint64_t _allocations,
          // double _cpus, int _mem)  //yuquan: overload constructor
  // : name(_name), share(_share), allocations(_allocations),
    // cpus_per_task(_cpus), mem_per_task(_mem) {}

  std::string name;
  double share;

  // double cpus_per_task = 1;  // yuquan: add cpus and mem fields
  // int mem_per_task = 1000;
...
public:
  ...
  // virtual void add(const std::string& name, double weight,
                   // const FrameworkInfo::Dvector dvector);
                   // yuquan: overload add here
  ...
  // yuquan: add total resources of the slave to the sorter
  virtual void addSlaveTotal(const SlaveID& slaveId,
      const Resources& resources);
  ...
  virtual std::vector<std::string> sortPerSlave(const SlaveID& slaveId);
  // yuquan: redeclare it again
...
private:
  ...
  // yuquan: add new function calculateMetric here
  double calculateMetric(const SlaveID& slaveId, std::string& name);
  ...
  // yuquan: add client dictionary to quick client look up
  // hashmap<std::string, Client> clientDict;

  // yuquan: since the per-slave resources kept in total_.resources is very
  // confusing, I would add a new hashmap to keep SlaveID->Resources map,
  // mapping SlaveID to slave.total (Resources) in slaves, which is a hashmap
  // defined in hierachical.hpp
  hashmap<SlaveID, Resources> slaveTotal;
