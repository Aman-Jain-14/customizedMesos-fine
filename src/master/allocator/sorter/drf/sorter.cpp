...
/*
void DRFSorter::add(const string& name, double weight, const FrameworkInfo::Dvector dvector) 	// yuquan: add this new function
{
  CHECK(!contains(name));
 
  Client client(name, 0, 0, dvector.cpus(), dvector.mem());
  clients.insert(client);	
...
void DRFSorter::add(const SlaveID& slaveId, const Resources& resources)
{
  if (!resources.empty()) {
    ...
    /*
    std::cout << "\n\n---> DRFSorter:: add"
              << "(" << slaveId.value() << ")" << std::endl;
    foreach (const Resource& resource, resources) {
      std::cout << resource.name() << ": " << resource.scalar().value()
                << std::endl;
    }
    */
    const Resources scalarQuantities =
...
// yuquan: implement the new function addSlaveTotal
void DRFSorter::addSlaveTotal(const SlaveID& slaveId,
         const Resources& resources)
{
  if (!resources.empty()) {
    slaveTotal[slaveId] = resources;
    std::cout << "\n\n---> DRFSorter::addSlaveTotal: "
              << slaveId.value() << resources << std::endl;
  }
}
...
// yuquan: implement sort() per slave. TODO(yuquan): implement
// calculateMetric(const SlaveID&, std::string&)
vector<string> DRFSorter::sortPerSlave(const SlaveID& slaveId) {
  // forced to recalculate metrics all over again,
  // if we want to keep a single Client for a framework
  LOG(INFO) << "---> DRFSorter::sortPerSlave: entering...";
  set<Client, DRFComparator> temp;
  foreach (Client client, clients) {
    client.share = calculateMetric(slaveId, client.name);
    LOG(INFO) << "==== Username: " << client.name
              << " share: " << client.share;
    temp.insert(client);
  }
  clients = temp;
...
// yuquan: implement calculateMetric
// TODO(yuquan): so far we only consider two resources, cpu and memory,
// the function might be changed if
// some other resources are included
double DRFSorter::calculateMetric(const SlaveID& slaveId, std::string& name) {
  LOG(INFO) << "---> DRFSorter::calculateMetric(): enter calculateMetric"
            << "(slave: " << slaveId.value() << " user: " << name << ")";
  // const Resources scalarQuantities = total_.resources[slaveId]
  //                                   .createStrippedScalarQuantity();
  const Resources& scalarQuantities = slaveTotal[slaveId];
  // Client& clnt = clientDict.at(name);
  if (scalarQuantities.empty()) {
    LOG(INFO) << "oops... total resource is empty!?";
  }
  std::string dominantResource; // name of dommiant resource
  double tmp = 0.0; // (allocated amount)/(total amount) for the
                        // temporary "champion"
  double denom = 1.0;

  foreach (const Resource& resource, scalarQuantities) {
    std::cout << "total resource on this slave "
              << resource.name() << " with quantity "
              << resource.scalar().value()  << std::endl;
    if (resource.name() == "cpus" && resource.scalar().value() != 0.0 &&
               allocations.at(name).resources.contains(slaveId)) {
      double ratio = allocations.at(name).resources.at(slaveId)
                  .cpus().get()/resource.scalar().value();
      std::cout << "occupied by this slave: "
                << allocations.at(name).resources.at(slaveId).cpus().get()
                << std::endl;
      std::cout << "ratio = " << ratio << std::endl;
      if (tmp <= ratio) {
        tmp = ratio;
        dominantResource = resource.name();
        denom = resource.scalar().value();
      }
    }
    if (resource.name() == "mem" && resource.scalar().value() != 0 &&
        allocations.at(name).resources.contains(slaveId)) {
      double ratio = (double)allocations.at(name).resources.at(slaveId).mem()
                  .get().megabytes()/resource.scalar().value();
      std::cout << "occupied by this slave: "
                << allocations.at(name).resources.at(slaveId).mem().get()
                << std::endl;
      std::cout << "ratio = " << ratio << std::endl;
      if (tmp <= ratio) {
        dominantResource = resource.name();
  ...
  return allocations[name].totals[dominantResource].value()/denom;
}
