...
HierarchicalAllocatorProcess::Framework::Framework(
  ...
  : roles(protobuf::framework::getRoles(frameworkInfo)),
    ...
    dvector(frameworkInfo.dvector()){}	// yuquan: add dvector in the initilization list
*/
HierarchicalAllocatorProcess::Framework::Framework(
    const FrameworkInfo& frameworkInfo)
    : roles(protobuf::framework::getRoles(frameworkInfo)),
    suppressed(false),
    capabilities(frameworkInfo.capabilities()),
    cpus(frameworkInfo.cpus()),
    mem(frameworkInfo.mem()){
    }
    ...
    const hashmap<FrameworkID, Resources>& used)
{
  ...
  // yuquan: update slave total resource in sorter

  foreachvalue (const Owned<Sorter>& frameworkSorter,
               frameworkSorters) {
    frameworkSorter->addSlaveTotal(slaveId, slave.total);
  }
  allocate(slaveId);
}
    ...
    const vector<Offer::Operation>& operations)
{
  ...
  /*
  std::cout << "\n\nhierachial.cpp: frameworkSorter->remove" << std::endl;
  ...
  */
  ...
  /*
  std::cout << "\n\nhierachial.cpp: frameworkSorter->add" << std::endl;
  foreach (const Resource& resource, updatedOfferedResources) {
    std::cout << resource.name() << ": " << resource.scalar().value()
             << std::endl;
  }
  */
    ...
    const Option<Filters>& filters)
{
  ...
  if (timeout.get() != Duration::zero()) {
    ...
    // added the following LOG(INFO) by yuquan
    ...
    // yuquan: add the following cout
    std::cout << "~~~~ Creating refused filter for resources"
              << unallocated  << std::endl;
  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    ...
    foreach (const string& role, roleSorter->sort()) {
      ...
      foreach (const string& frameworkId_,
              frameworkSorter->sortPerSlave(slaveId)) {
      // change sort() to sortPerSlave(slaveId)
        std::cout << "("  << slaveId.value() << "," << role << ","
                  << frameworkId_ << ")" << std::endl;
        FrameworkID frameworkId;
        frameworkId.set_value(frameworkId_);
        ...
        if (!allocatable(resources)) {
          std::cout << "~~~~~~~~ (1)Resources unallocatable!!!"
                    << resources << std::endl;
          break;
        }
        ...
        if (!allocatable(resources)) {
          std::cout << "~~~~~~~~ (2)Resources unallocatable!!!"
                    << resources << std::endl;
          continue;
        }
        ...
        // Added by Aman
        // resources.cpus() = framework.cpus;

        // Let's try this man !!

        std::string s = "";
        auto x_str = std::to_string(framework.cpus);
        s = s + "cpus:"+x_str;
        auto y_str = std::to_string(framework.mem);
        s = s + ";mem:" + y_str;

        vector<Resource> resources1;

          foreach (const string& token, strings::tokenize(s, ";")) {
            ...
            if (openParen == string::npos) {
              ...
              if (closeParen == string::npos || closeParen < openParen) {
                LOG (INFO) << "Bad value for resources, mismatched parentheses in ";
              }
            ...
            resources1.push_back(resource.get());
          }

          Try<vector<Resource>> resources2 = resources1;

          Resources result;

          // Validate individual Resource objects.
          foreach(const Resource& resource, resources2.get()) {
            result.add(resource);
          }
        ...
        foreach (const Resource& resource, resources) {
              LOG (INFO) << "total resource on this slave "
              << resource.name() << " with quantity "
              << resource.scalar().value()  << std::endl;

        }
        LOG (INFO) << "Name of Resource = " << resources.get("cpus");

        // Added by Aman
        LOG (INFO) << " Showing D Vector of selected framework = CPU : " << framework.cpus
                   << " Mem : " << framework.mem ;

        scalarQuantity = resources.createStrippedScalarQuantity();
    ...
    const Resources& resources) const
{
  ...
  foreach (OfferFilter* offerFilter, agentFilters->second) {
    if (offerFilter->filter(resources)) {
      ...
      // added by yuquan
      std::cout << "Filtered offer with " << resources
                << " on agent " << slaveId
                << " for role " << role
                << " of framework " << frameworkId << std::endl;
    ...
    const string& role)
{
  ...
  if (!roles.contains(role)) {
    ...
    // yuquan: add already-existed slaves to the new sorter
    foreachpair (const SlaveID& slaveId, const Slave& slave, slaves) {
      frameworkSorters.at(role)->addSlaveTotal(slaveId, slave.total);
    }
  ...
  // frameworkSorters.at(role)->add(frameworkId.value(),
  // 1.0, frameworks.at(frameworkId).dvector);
  // yuquan: replace with this to pass d-vector
}
